...
export function ChatPanel({ initialSummary, logContext }: ChatPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-900/30 border-l border-zinc-800 w-96">
      {/* Header */}
      <div className="p-4 border-b border-zinc-800 flex items-center gap-2 bg-zinc-950">
        <Sparkles className="w-4 h-4 text-purple-400" />
        <h3 className="font-semibold text-zinc-100 text-sm">AI Investigator</h3>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar" ref={scrollRef}>
        ...
        <div className="space-y-4 animate-in fade-in duration-500">
            <div className={cn(
                ...
                initialSummary.severity === 'high' ? "bg-red-500/10 border-red-500/20 text-red-200" :
            ...
            <div className="space-y-2">
                ...
                <div className="flex gap-2 text-sm text-indigo-300 bg-indigo-500/5 p-3 rounded border border-indigo-500/10 items-start">
                    <CheckCircle className="w-4 h-4 text-indigo-400 flex-shrink-0 mt-0.5" />
                    <span>{initialSummary.recommendation || "No specific recommendation provided."}</span>
                </div>
            </div>

            <div className="border-b border-zinc-800/50 pt-2" />
        </div>
        ...
        {messages.map((msg, i) => (
          <div key={i} className={cn("flex gap-3", msg.role === 'user' ? "flex-row-reverse" : "")}>
            <div className={cn(
                ...
                msg.role === 'user' ? "bg-zinc-700" : "bg-indigo-500/20"
            )}>
                {msg.role === 'user' ? <User className="w-3 h-3 text-zinc-300" /> : <Bot className="w-3 h-3 text-indigo-400" />}
            </div>
            <div className={cn(
                "rounded-lg p-3 text-sm max-w-[85%] whitespace-pre-wrap leading-relaxed shadow-sm",
                msg.role === 'user' ? "bg-zinc-800 text-zinc-100" : "bg-zinc-950 border border-zinc-800 text-zinc-300"
            )}>
                {msg.content}
            </div>
          </div>
      ...
      <div className="p-4 border-t border-zinc-800 bg-zinc-950">
        <div className="relative">
            <input 
                ...
                className="w-full bg-zinc-900 border border-zinc-800 rounded-md py-2.5 pl-3 pr-10 text-sm text-zinc-100 focus:ring-1 focus:ring-indigo-500 outline-none transition-all placeholder:text-zinc-600"
            />
            <button 
                onClick={handleSend}
                disabled={loading || !input.trim()}
                className="absolute right-2 top-2.5 text-zinc-500 hover:text-indigo-400 disabled:opacity-50 transition-colors"
            >
